...
補足（実装状況メモ）
- v0では前景（フォアグラウンド）でのスキャン/アドバタイズのみ対応（バックグラウンドは後続）。
...
* **再会時の通知（今後実装予定）**: マッチ済みの2人が物理的に再度すれ違った場合にのみ、両者へ通知を送ります。
...
#### 3.7. 通知機能（要件変更）
* **再会時のみ通知**: マッチ済みの相手と再度すれ違った瞬間に、両者にプッシュ通知を送ります（例：「〇〇さんとまた会いましたね！」）。
* **マッチ成立時の通知は行いません**（今後も行わない方針）。
* **その他のタイミング**（いいねされた時、最初のすれ違い時など）での通知は行いません。
