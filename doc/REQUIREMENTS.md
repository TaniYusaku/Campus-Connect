...
* **ユーザー認証**
    * 現行MVPはメール/パスワード認証を採用し、京都産業大学ドメイン（`*.kyoto-su.ac.jp`）のメールアドレスのみ受け付けます（`/api/auth/*`）。
    * 将来的にドメインの拡張や追加確認フローの導入を検討します。
...
#### 3.2. BLEによるすれ違い検知
* **検知条件**: RSSIが概ね -85dBm 以上のデバイスを検出した際に観測イベントを送信します。サーバー側で5分以内の相互観測（互いに観測し合う）を検出できた場合のみ「すれ違い」として確定します。
