...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  Future<void> _pickAndUploadPhoto() async {
    ...
    try {
      ...
      final info = await api.requestProfilePhotoUploadUrl(contentType: contentType);
      ...
      if (putResp.statusCode >= 200 && putResp.statusCode < 300) {
        final updated = await api.confirmProfilePhoto(objectPath: info.objectPath);
        if (updated != null) {
        ...
        throw Exception('Upload failed: ${putResp.statusCode} ${putResp.reasonPhrase} | $bodyText');
      }
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _initFields(User u) {
    ...
    _selectedFaculty = (u.faculty != null && kFacultyOptions.contains(u.faculty)) ? u.faculty : '未設定';
    if (u.grade != null) {
    ...
    _selectedGender = (u.gender != null && kGenderOptions.contains(u.gender)) ? u.gender : '未設定';
    _initialized = true;
  }
  ...
  Future<void> _save() async {
    ...
    if (_selectedGradeStr != null && _selectedGradeStr != '未設定') {
      ...
      else gradeInt = int.tryParse(_selectedGradeStr!);
    }
    final updated = await api.updateMe(
      ...
      gender: (_selectedGender == null || _selectedGender == '未設定') ? null : _selectedGender,
    );
                ...
                child: Form(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      validator: (v) => (v == null || v.isEmpty || v == '未設定') ? '性別は必須です' : null,
                      autovalidateMode: AutovalidateMode.onUserInteraction,
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: [
                        const DropdownMenuItem<String>(value: null, child: Text('テンプレートを選択しない')), 
                        ...kBioTemplates.map((s) => DropdownMenuItem<String>(value: s, child: Text(s))),
                      ],
                      onChanged: (v) {
                      ...
                      decoration: const InputDecoration(labelText: '自己紹介テンプレート'),
                    ),
                    ...
                    TextFormField(
                      controller: _xCtl,
                      decoration: const InputDecoration(labelText: 'X (旧Twitter) ID'),
                    ),
                    const SizedBox(height: 8),
                    TextFormField(
                      controller: _igCtl,
