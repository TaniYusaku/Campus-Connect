...
本アプリケーションは以下の主要なライブラリ（パッケージ）を利用して構築されています。

| カテゴリ | ライブラリ名 | 役割 |
| :--- | :--- | :--- |
| **状態管理** | `flutter_riverpod` | アプリ全体の認証状態やユーザー情報などを管理する。 |
| **API通信** | `dio` | バックエンドAPIとの通信（ログイン、ユーザー情報取得など）を担う。 |
| **認証** | `firebase_auth` | Firebase Authenticationと連携し、ユーザー認証機能を実現する。 |
| **セキュアストレージ** | `flutter_secure_storage` | 認証トークンなど、機密性の高い情報をデバイス内に安全に保存する。 |
| **OS権限管理** | `permission_handler` | BLEやプッシュ通知など、OSの許可が必要な機能へのアクセスをユーザーに要求する。 |
| **簡易データ保存** | `shared_preferences` | アプリの設定など、軽量なデータを簡易的に保存する。 |
