...
2. すれ違い判定
- 基本: RSSI がしきい値（初期値: ≥ -85dBm）を超えたら観測イベントを送信し、**サーバが5分以内の双方向観測を検出した場合のみ Encounter を確定**します（`/api/encounters/observe` 発火→サーバ内で相互観測を確認）。
- フィルタ: 現状はRSSI閾値のUI調整なし（コード上は固定 -85dBm）。同一端末は5分デバウンス（相互観測成立後も5分）で過剰送信を避ける。
...
- RSSI 閾値やフィルタ条件は当面固定値（-85dBm）で開始し、必要になればUI調整を後続で検討する方針。
