...
* **POST `/encounters`**
    * **利用する場面:** アプリがフォアグラウンドでBLEスキャンを行い、他のユーザーを検知したタイミングで継続的に呼び出される。
    * **必要な理由:** アプリの根幹機能である「すれ違い」を記録するため。このログがすべてのマッチングの元になる。
    * **備考:** 最も呼び出し頻度が高くなるAPI。サーバーは高い負荷を効率的に処理できるよう設計する必要がある。`recentEncounters` の該当ドキュメントに `lastEncounteredAt` を更新しつつ `encounterCount` をインクリメントする。
...
* **GET `/encounters`**
    ...
    * **備考:** パフォーマンスのため、必ずページネーション（例: `?page=1&limit=20`）を実装すること。レスポンスには `lastEncounteredAt`（ISO8601文字列）、`encounterCount`（接触回数）、`isFriend`（友達状態）を含め、クライアント側で通知ロジックに利用できるようにする。
